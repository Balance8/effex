...
function isPrimaryKey(field: Field): boolean {
  return field.attributes?.some(attr => attr.name === 'id') ?? false
}

function mapPrismaTypeToEffect(prismaType: string, isArray: boolean): string {
...
function _generateSelectSchema(model: Model): string {
...
function generateServiceFile(model: Model): string {
...
export function generateEffectServices(schemaPath: string, outputDir: string): void {
