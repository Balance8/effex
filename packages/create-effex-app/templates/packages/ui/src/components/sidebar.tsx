...
function SidebarProvider({
  ...
  const setOpen = React.useCallback(
    (openValue: boolean | ((value: boolean) => boolean)) => {
      const openState = typeof openValue === 'function' ? openValue(open) : openValue
      if (setOpenProp) {
...
function SidebarMenuSkeleton({
  ...
  const MAX_WIDTH_RANGE = 40
  // Random width between 50 to 90%.
  const width = React.useMemo(
    () => `${Math.floor(Math.random() * MAX_WIDTH_RANGE) + MIN_WIDTH_PERCENT}%`,
    []
  )
